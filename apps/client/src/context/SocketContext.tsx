...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [playerId] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
        // Use sessionStorage instead of localStorage so multiple tabs = multiple players
        // But reloads still preserve the ID for reconnection.
        let storedId = sessionStorage.getItem('royal_tens_uid');
        if (!storedId) {
            storedId = generateUUID();
            sessionStorage.setItem('royal_tens_uid', storedId);
        }
        return storedId;
    }
    return null;
